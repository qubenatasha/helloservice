...
class HelloServiceService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = HelloService.query.get(entity_id)
        if data is None:
            raise HelloServiceServiceError(
                'helloservice {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = HelloService.query.filter(
            HelloService.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = HelloService.query.get(entity_id)  # HelloService is a mongo class
        if record is None:
            raise HelloServiceServiceError(
                'helloservice {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
