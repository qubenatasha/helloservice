...
from qube.src.api.swagger_models.helloservice import HelloServiceModel # noqa: ignore=I100
from qube.src.api.swagger_models.helloservice import HelloServiceModelPost # noqa: ignore=I100
from qube.src.api.swagger_models.helloservice import HelloServiceModelPostResponse # noqa: ignore=I100
from qube.src.api.swagger_models.helloservice import HelloServiceModelPut # noqa: ignore=I100
...
from qube.src.api.swagger_models.response_messages import (
    ...
    put_response_msgs)
from qube.src.commons.error import HelloServiceServiceError
...
class HelloServiceItemController(Resource):
    @swagger.doc(
        {
            'tags': ['HelloService'],
            'description': 'HelloService get operation',
    ...
    def get(self, authcontext, entity_id):
        """gets an helloservice item that omar has changed
        """
        try:
            LOG.debug("Get details by id %s ", entity_id)
            data = HelloServiceService(authcontext['context'])\
                .find_by_id(entity_id)
            clean_nonserializable_attributes(data)
        except HelloServiceServiceError as e:
            LOG.error(e)
        ...
        return HelloServiceModel(**data), 200

    @swagger.doc(
        {
            'tags': ['HelloService'],
            'description': 'HelloService put operation',
    ...
    def put(self, authcontext, entity_id):
        """
        updates an helloservice item
        """
        try:
            model = HelloServiceModelPut(**request.get_json())
            context = authcontext['context']
            HelloServiceService(context).update(model, entity_id)
            return EMPTY, 204
        except HelloServiceServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
    ...
    @swagger.doc(
        {
            'tags': ['HelloService'],
            'description': 'HelloService delete operation',
    ...
    def delete(self, authcontext, entity_id):
        """
        Delete helloservice item
        """
        try:
            HelloServiceService(authcontext['context']).delete(entity_id)
            return EMPTY, 204
        except HelloServiceServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
...
class HelloServiceController(Resource):
    @swagger.doc(
        {
            'tags': ['HelloService'],
            'description': 'HelloService get operation',
    ...
    def get(self, authcontext):
        """
        gets all helloservice items
        """
        LOG.debug("Serving  Get all request")
        list = HelloServiceService(authcontext['context']).get_all()
    ...
    @swagger.doc(
        {
            'tags': ['HelloService'],
            'description': 'HelloService create operation',
    ...
    def post(self, authcontext):
        """
        Adds a helloservice item.
        """
        try:
            model = HelloServiceModelPost(**request.get_json())
            result = HelloServiceService(authcontext['context'])\
                .save(model)

            response = HelloServiceModelPostResponse()
